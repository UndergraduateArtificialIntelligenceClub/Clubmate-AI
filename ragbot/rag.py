...
def create_embeddings(model_name: str) -> HuggingFaceEmbeddings:
    """Create HuggingFace embeddings with CPU (GPU support TODO)."""
    return HuggingFaceEmbeddings(
        model_name=model_name,
        model_kwargs={'device': 'cpu'},  # TODO: Add GPU detection
...
class RAGSystem:
    ...
    def __init__(self):
        ...
        # Initialize embeddings using langchain_huggingface
        self.embeddings = create_embeddings(RAGConfig.EMBEDDING_MODEL)
    ...
    def load_document(self, file_path: str) -> List[LCDocument]:
        ...
        if extension in ['.txt', '.md', '.markdown']:
            ...
            if PyMuPDFLoader is None:
                raise ImportError("PyMuPDFLoader not available. Install pymupdf package.")
    ...
    def ingest_file(self, file_path: str) -> bool:
        ...
        try:
            # Condensed: one log per ingestion
...
Answer with citations:"""
    ...
    def reset(self) -> bool:
        ...
        """
        # TODO: Fix Windows file locking bug - when ChromaDB is in use by another process
        # (e.g., Discord bot), shutil.rmtree fails with:
        # [WinError 32] The process cannot access the file because it is being used by another process
        # Possible solutions:
        # 1. Use vector_store.delete() to clear documents instead of deleting files
        # 2. Close ChromaDB client before deletion
...
def rag_retrieve(
    ...
    Unlike rag_query(), this returns raw chunks from the vector store
    without invoking an LLM. Useful for injecting context into another
    model's prompt.
