...
class TestMarkdownToPlainText:
    def test_strips_heading_markers(self):
        ...
        assert _markdown_to_plain_text("### Subtitle") == "Subtitle"
        assert _markdown_to_plain_text("# H1") == "H1"

    def test_converts_bold(self):
        assert _markdown_to_plain_text("**bold text**") == "bold text"
        assert _markdown_to_plain_text("__bold alt__") == "bold alt"

    def test_converts_italic(self):
        assert _markdown_to_plain_text("*italic*") == "italic"
        assert _markdown_to_plain_text("_italic alt_") == "italic alt"

    def test_converts_inline_code(self):
        assert _markdown_to_plain_text("`code`") == "code"

    def test_converts_links(self):
        result = _markdown_to_plain_text("[Google](https://google.com)")
        assert "Google" in result
        assert "https://google.com" in result

    def test_converts_list_markers(self):
        assert "- Item" in _markdown_to_plain_text("* Item")
        assert "- Item" in _markdown_to_plain_text("+ Item")
        assert "- Item" in _markdown_to_plain_text("1. Item")

    def test_removes_horizontal_rules(self):
        result = _markdown_to_plain_text("---")
        assert result == ""
    ...
    def test_none_input(self):
        assert _markdown_to_plain_text(None) is None
